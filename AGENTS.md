...
A personal command center for orchestrating Codex sessions, plus scheduled
automations that run unattended.

## Layout

```
apps/orchestrator   Node/TS service. Owns SQLite, the Codex Agent SDK, the scheduler,
...
- **Never surface cost/dollar figures in the UI.** Sessions run on a Codex subscription;
