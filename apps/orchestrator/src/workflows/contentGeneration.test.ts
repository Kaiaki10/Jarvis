...
beforeAll(() => {
  const dir = mkdtempSync(join(tmpdir(), "jarvis-workflows-"));
...
describe("campaign content generation", () => {
  ...
  it("treats the campaign brief as authoritative instead of exploring the project", async () => {
    const { workflowGenerationPrompt } = await import("./contentGeneration.js");
    const repo = await import("../db/workflowRepo.js");
    const campaign = repo.createWorkflow({
      name: "Focused brief",
    ...
    const prompt = workflowGenerationPrompt({ campaign, count: 1, formats: ["social_post"], channels: ["linkedin"] });
  ...
  it("turns a successful generation run into durable drafts", async () => {
    ...
    const { reconcileWorkflowGeneration } = await import("./contentGeneration.js");
    const campaign = repo.createWorkflow({
      name: "Launch",
    ...
    repo.createWorkflowGenerationRun({ workflowId: campaign.id, sessionId: session.id, requestedCount: 1 });

    expect(reconcileWorkflowGeneration({
    ...
    expect(repo.getWorkflowGenerationRunBySession(session.id)?.status).toBe("completed");
  });
  ...
  it("fails closed when Jarvis targets an unapproved channel", async () => {
    ...
    expect(repo.getWorkflowGenerationRunBySession(session.id)).toMatchObject({ status: "failed" });
  });
});
